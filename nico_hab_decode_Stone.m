function [hab_out] = nico_hab_decode_Stone(dirname)
...
for file =1:length(matfile')
    ...
    tastes = ["Sucrose,","H2O,","H20,","WATER,","NaCl,","QHCl,","Sacchrin,"]; num_var = ["1","2","2","2","3","4","5"]; 
    ...
    test_day = matfile(file).name(strfind((matfile(file).name),'HAB'):strfind((matfile(file).name),'HAB')+3);    
   
    %Place in structure under animal name
    combined_OR_hab.([test_day]).([animal]).('data_headers') = headers(1,[1 2 4 5 6 7 8 9 10 11]);
    combined_OR_hab.([test_day]).([animal]).('trial_data') = full_data;
    combined_OR_hab.([test_day]).([animal]).('sessions_tastes_decode') = vertcat(tastes,num_var);
    ...
    %Place in structure under animal name
    combined_OR_hab.([test_day]).([animal]).('licks_session') = lick_trials;
    combined_OR_hab.([test_day]).([animal]).('licks_per_trial') = licks;
    combined_OR_hab.([test_day]).([animal]).('lat_first_trial') = lat_start;
    combined_OR_hab.([test_day]).([animal]).('cummulative_latency_trial') = cum_sum_lat'; %latenciesXtrials

end

hab_out = combined_OR_hab;